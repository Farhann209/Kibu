...
import { Button, ButtonGroup, Link as NextUILink, Image } from "@nextui-org/react" // Import UI components from NextUI
    ...
    <div>
        ...
        <div className='ml-4'>
          <ButtonGroup>
            <Button className='border-r border-black hover:bg-blue-400 font-semibold'>
              <NextUILink href={'../listing'} className='text-black'> Nayabazar </NextUILink> {/* Link to Nayabazar listings */}
            </Button>
            <Button className='hover:bg-blue-400 font-semibold'>
              <NextUILink href={'../listing/bookRoomsBoudha'} className='text-black'> Boudha </NextUILink> {/* Link to Boudha listings */}
            </Button>
          </ButtonGroup>
        </div>
      </div>
      ...
      <div>
        <Grid templateColumns='repeat(3, 1fr)' className="ml-4" gap={1}>
          {listingList.filter(item => item.listingLocation === 'Nayabazar').map((item) => ( // Filter listings by location 'Nayabazar'
            <GridItem className='w-[100%]' key={item._id}>
              <Link href={`/listing/${item._id}`}> {/* Link to the specific listing's detail page */}
                <Card style={{ height: '95%' }} className="m-3">
                  <CardHeader className="pb-0 pt-2 px-4 flex-col items-start">
                    <Image
                      ...
                      width={400}
                      height={'auto'} // Ensure width is set for the Image component
                    />
                  </CardHeader>
